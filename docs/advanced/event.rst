...
The clashy.py events client does exactly that: making requests to the API every X seconds,
...
Decorators are a simple, easy way to interact with clashy.py events. They are grouped into 4 categories:
...
latency between when the event happens in game and when clashy.py reports it.
